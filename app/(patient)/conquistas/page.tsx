...
export default async function PerfilPage() {
  ...
  return (
    <div className="space-y-6">
      <p className="text-[10px] text-[#9a8c70] uppercase tracking-widest font-medium">Perfil</p>
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        <div className="text-center">
          ...
          <p className="text-[#9a8c70] text-sm">{level.emoji} {getLevelName(level, sex)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          { label: 'XP Total', value: totalXP, color: 'text-yellow-400' },
          { label: 'Streak',   value: `${streak}🔥`, color: 'text-orange-400' },
          { label: 'Conquistas', value: earnedMap.size, color: 'text-yellow-400' },
        ].map((s) => (
          <div key={s.label} className="bg-[#1a1610] border border-[#2a2419] rounded-xl py-3 text-center">
            <p className={`text-lg font-bold ${s.color}`}>{s.value}</p>
            <p className="text-[10px] text-[#9a8c70] uppercase tracking-wider mt-0.5">{s.label}</p>
          </div>
        ))}
      </div>
      ...
      {(patient as any)?.objective && (
        <div className="bg-[#1a1610] border border-[#2a2419] rounded-xl px-4 py-3">
          <p className="text-[10px] text-[#9a8c70] uppercase tracking-widest mb-1">Objetivo</p>
      ...
      {/* Conquistas */}
      <div>
        <p className="text-[10px] text-[#9a8c70] uppercase tracking-widest font-medium mb-3">
          Conquistas & Medalhas
        </p>
            ...
            return (
              <div
                ...
                className={`rounded-xl px-3 py-3 border text-center transition-all ${
                  earned
                    ? 'bg-[#1a1610] border-yellow-500/30'
                    : 'bg-[#0d0e1c] border-[#2a2419] opacity-40'
                }`}
              >
                <div className="text-2xl mb-1">{earned ? ach.emoji : '🔒'}</div>
                <p className="text-xs font-semibold leading-tight">{ach.label}</p>
                <p className="text-[10px] text-[#9a8c70] mt-0.5">{ach.description}</p>
                {earned && earnedAt && (
      ...
      <form action="/auth/logout" method="post" className="pt-2">
        <button
          type="submit"
          className="w-full bg-[#1a1610] border border-[#2a2419] hover:border-[#2a2b50] text-[#8892b0] hover:text-white rounded-xl py-3 text-sm font-medium transition-colors uppercase tracking-wider"
        >
          Sair da conta
        </button>
      </form>
