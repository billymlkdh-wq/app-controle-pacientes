...
export function ComunidadeFeed({ posts, myPatientId, myName, myAvatarUrl, currentTipo }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Title */}
      <p className="text-[10px] text-[#9a8c70] uppercase tracking-widest font-medium">
        Feed da Comunidade ✨
      </p>

      {/* New post box */}
      {myPatientId && (
        <div className="bg-[#1a1610] border border-[#2a2419] rounded-2xl p-3 flex gap-3">
          ...
          <div className="flex-1 flex gap-2">
            ...
            <button
              ...
              className="bg-yellow-500 hover:bg-yellow-600 disabled:opacity-40 rounded-xl w-9 h-9 flex items-center justify-center flex-shrink-0 transition-colors"
            >
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
              currentTipo === f.value
                ? 'bg-yellow-500 text-black shadow-[0_0_10px_rgba(245,197,24,0.35)]'
      ...
      {localPosts.map((post) => {
        ...
        return (
          <div
            key={post.id}
            className="bg-[#1a1610] border border-[#2a2419] rounded-2xl overflow-hidden border-l-2 border-l-yellow-500/40"
          >
            {/* Post header */}
            <div className="flex items-center justify-between px-4 pt-3 pb-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#9a8c70]">{timeAgo(post.createdAt)}</p>
                </div>
              </div>
              <span className="text-[10px] bg-[#0f0d0a] border border-[#2a2419] rounded-full px-2 py-0.5 text-[#9a8c70]">
                {typeLabel}
              </span>
            </div>
            ...
            {/* Reactions */}
            <div className="flex items-center gap-4 px-4 pb-3 pt-1 border-t border-[#2a2419]">
              <button
                onClick={() => toggleReact(post.id, post.iReacted)}
                disabled={!myPatientId || isMe}
                className={`flex items-center gap-1.5 text-xs transition-colors ${
                  post.iReacted ? 'text-yellow-400' : 'text-[#9a8c70] hover:text-yellow-400'
                } disabled:opacity-40`}
              >
                <Heart className={`h-4 w-4 ${post.iReacted ? 'fill-yellow-400' : ''}`} />
                <span>{post.reactionCount}</span>
              </button>
              <button
                onClick={() => setCommenting(commenting === post.id ? null : post.id)}
                className="flex items-center gap-1.5 text-xs text-[#9a8c70] hover:text-white transition-colors"
              >
            ...
            {commenting === post.id && (
              <div className="flex gap-2 px-4 pb-3">
                <input
                  ...
                  className="flex-1 bg-[#0f0d0a] border border-[#2a2419] rounded-xl px-3 py-2 text-sm text-white placeholder:text-[#9a8c70] outline-none focus:border-yellow-500/50"
                />
                <button className="bg-yellow-500 rounded-xl w-9 h-9 flex items-center justify-center flex-shrink-0">
